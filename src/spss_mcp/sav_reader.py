...
def _check_file(file_path: str) -> Path:
    ...
    if p.suffix.lower() not in (".sav", ".por", ".zsav"):
        raise ValueError(
            f"Unsupported file type: {p.suffix}. Expected .sav, .zsav, or .por"
        )
    return p
...
def format_metadata_as_markdown(meta_dict: dict) -> str:
    ...
    lines.append(
        tabulate(
            rows, headers=["Name", "Label", "Type", "Value Labels"], tablefmt="github"
        )
    )
    ...
    if val_labels:
        ...
        for var, mapping in val_labels.items():
            ...
            lines.append(
                tabulate(vl_rows, headers=["Code", "Label"], tablefmt="github")
            )
...
def format_summary_as_markdown(summary: dict) -> str:
    ...
    if file_label:
        lines.append(f"**File label:** {file_label}\n")
    lines.append(
        f"**Cases:** {summary['n_cases']} | **Variables:** {summary['n_variables']}\n"
    )
