...
async def run_syntax(
    ...
    if not get_spss_python():
        return {
            ...
            "output_raw": "",
            "error": (
                "SPSS Python interpreter not found (detected engine: "
                f"{stats_exe}). The SPSS-bundled Python3 runtime is required "
                "to drive the engine."
            ),
            "warnings": [],
        }

    # ---- P4 security gate: dangerous commands, path allowlist, audit ----
    from spss_mcp.security import (
        audit,
        check_syntax_safety,
        summarize_syntax,
        validate_data_file,
    ...
    if blocked:
        ...
        return {
            ...
            "last_error_level": 0,
            "process_returncode": None,
            "parsed_errors": [f"dangerous syntax: {', '.join(blocked)}"],
            "parsed_warnings": [],
            "output_markdown": "",
            "output_raw": "",
            "error": f"Syntax blocked: dangerous commands {', '.join(blocked)}.",
            "warnings": [],
            "tables": [],
    ...
    if not path_error:
        for m in re.finditer(r"GET FILE='([^']+)'", syntax):
            path_error = validate_data_file(m.group(1))
            if path_error:
                break
    if path_error:
        audit(
    ...
    syntax_file = (
        results_dir / f"spss_syntax_{run_id}.sps" if save_syntax_file else None
    )
    viewer_file = (
        results_dir / f"spss_viewer_{run_id}.spv" if save_viewer_output else None
    )
    ...
    syntax_issue_warnings = (
        _extract_syntax_issue_warnings(raw_output) if raw_output else []
    )
    ...
    if not success:
        if timed_out:
            ...
            error_msg = (
                "; ".join(parsed_errors)
                if parsed_errors
                else f"SPSS reported fatal error level {err_level}"
            )
    ...
    viewer_output_file = (
        str(viewer_file) if viewer_ok and viewer_file and viewer_file.exists() else None
    )
    syntax_output_file = (
        str(syntax_file) if syntax_file and syntax_file.exists() else None
    )
    ...
    tables = parse_tables(raw_output)

    from spss_mcp.security import audit, summarize_syntax
    ...
    return {
        ...
        "warnings": warnings,
        "tables": tables,
        "summary": summarize_analysis(raw_output, tables),
        "files": [f for f in (viewer_output_file, syntax_output_file) if f],
    }
...
def build_regression_syntax(
    ...
    diag = (
        "COEFF OUTS R ANOVA COLLIN TOL" if include_diagnostics else "COEFF OUTS R ANOVA"
    )
    return (
        f"GET FILE='{file_path}'.\n"
        f"REGRESSION\n"
...
def build_t_test_syntax(
    ...
    if test_type == "one_sample":
        ...
        if not grouping_variable:
            raise ValueError(
                "grouping_variable is required for independent samples t-test"
            )
        return (
            f"GET FILE='{file_path}'.\n"
...
def build_compute_scale_syntax(
    ...
    if reverse_items:
        if reverse_min is None or reverse_max is None:
            raise ValueError(
                "reverse_min and reverse_max are required when reverse_items is provided"
            )
        for var in reverse_items:
            reverse_lines += f"IF (NOT MISSING({var})) {var} = ({reverse_max} + {reverse_min}) - {var}.\n"
...
def build_nonparametric_syntax(
    ...
    if test == "mann_whitney":
        ...
        if not group_values or len(group_values) != 2:
            raise ValueError(
                "group_values must contain exactly 2 values for mann_whitney"
            )
        return (
            f"GET FILE='{file_path}'.\n"
            f"NPAR TESTS\n"
    ...
    raise ValueError(
        "Unsupported test_type. Use: mann_whitney, wilcoxon, kruskal_wallis"
    )


def build_mediation_syntax(
    file_path: str,
    x: str,
    mediator: str,
    y: str,
) -> str:
    """Three-step (Baron & Kenny) mediation regression syntax.

    Step 1: M ~ X (path a); Step 2: Y ~ X (total effect c);
    Step 3: Y ~ X + M (paths c' and b).  Coefficients are parsed from the
    output to compute the indirect effect a*b and a Sobel test.
    """
    return (
        f"GET FILE='{file_path}'.\n"
        f"REGRESSION /STATISTICS=COEFF\n"
        f"  /DEPENDENT={mediator}\n"
        f"  /METHOD=ENTER {x}.\n"
        f"REGRESSION /STATISTICS=COEFF\n"
        f"  /DEPENDENT={y}\n"
        f"  /METHOD=ENTER {x}.\n"
        f"REGRESSION /STATISTICS=COEFF\n"
        f"  /DEPENDENT={y}\n"
        f"  /METHOD=ENTER {x} {mediator}.\n"
    )


def build_moderation_syntax(
    file_path: str,
    x: str,
    moderator: str,
    y: str,
) -> str:
    """Mean-centred moderation regression syntax (Y ~ X + W + X*W).

    The interaction term is created after centring both predictors with
    AGGREGATE column means; the interaction coefficient tests moderation.
    """
    return (
        f"GET FILE='{file_path}'.\n"
        f"AGGREGATE /OUTFILE=* MODE=ADDVARIABLES /BREAK=\n"
        f"  /{x}_mean=MEAN({x}) /{moderator}_mean=MEAN({moderator}).\n"
        f"COMPUTE {x}_c = {x} - {x}_mean.\n"
        f"COMPUTE {moderator}_c = {moderator} - {moderator}_mean.\n"
        f"COMPUTE interaction = {x}_c * {moderator}_c.\n"
        f"REGRESSION /STATISTICS=COEFF\n"
        f"  /DEPENDENT={y}\n"
        f"  /METHOD=ENTER {x}_c {moderator}_c interaction.\n"
    )
