...
from spss_mcp.codex_config import (
    build_codex_server_block,
...
def main():
    ...
    codex_parser = subparsers.add_parser(
        "configure-codex",
        help="Detect SPSS and automatically update Codex config.toml",
    )
    codex_parser.add_argument(
        "--config-file",
        help="Explicit Codex config.toml path to update",
    )
    ...
    if args.command == "version":
        ...
        print(f"SPSS Studio MCP v{__version__}")
        sys.exit(0)
        ...
        print(
            f"pyreadstat : {'OK v' + caps['pyreadstat_version'] if caps['pyreadstat'] else 'NOT FOUND  (pip install pyreadstat)'}"
        )
        print(
            f"pandas     : {'OK v' + caps['pandas_version'] if caps['pandas_version'] else 'NOT FOUND'}"
        )
        if caps["spss"]:
        ...
        print(f"=== SPSS Studio MCP v{__version__} Setup Info ===")
        ...
        result = configure_codex_settings(config_path)
        print("=== Codex configuration updated ===")
        ...
        print(build_codex_server_block(result["entry"]))
        print()
        print("Restart Codex to load the updated MCP server.")
        sys.exit(0)
