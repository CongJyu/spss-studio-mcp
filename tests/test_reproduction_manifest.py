...
def test_reproduction_manifest_cases_execute_successfully():
    manifest = json.loads(MANIFEST_PATH.read_text(encoding="utf-8"))
    asyncio.run(_run_cases(manifest))


async def _run_cases(manifest: list) -> None:
    from spss_mcp.spss_engine import get_engine

    engine = get_engine()
    try:
        # All cases share one event loop, mirroring how a real MCP client drives
        # the server: the persistent SPSS engine is started once and reused.
        # Spawning a fresh loop per case would leave the engine bound to a
        # now-closed loop (its pipes raise "attached to a different loop").
        for case in manifest:
            if case.get("syntax_kind") == "raw":
                ...
                rendered = await tool(
                    file_path=_resolve_data_file(case["file_path"]), **case["params"]
                )
        ...
        # Release the engine inside the same loop so the child process is reaped
        # before the loop closes (avoids a stray "Loop ... that handles pid ... is
        # closed" warning) and so the SPSS license seat is not held after the test.
        try:
            await engine.stop()
        except Exception:
            pass
