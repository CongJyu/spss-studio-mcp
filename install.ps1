# spss-studio-mcp 一键安装脚本 (PowerShell)
# 使用方法：右键点击 -> 使用 PowerShell 运行

Write-Host "========================================" -ForegroundColor Cyan
Write-Host "  spss-studio-mcp 一键安装脚本" -ForegroundColor Cyan
Write-Host "========================================" -ForegroundColor Cyan
Write-Host ""
...
Write-Host ""
Write-Host "[1/4] 安装 spss-studio-mcp..." -ForegroundColor Yellow
try {
...
$statusOutput = spss-studio-mcp status 2>&1
...
Copy-Item -Recurse -Force "skills\spss-studio-mcp-guard" "$skillsDir\spss-studio-mcp-guard"
Write-Host "[✓] 技能已安装到 $skillsDir" -ForegroundColor Green

Write-Host ""
Write-Host "[4/4] 自动配置 Claude Code..." -ForegroundColor Yellow
try {
    $configOutput = spss-studio-mcp configure-claude 2>&1
    Write-Host $configOutput
    Write-Host "[✓] Claude Code 配置已更新" -ForegroundColor Green
} catch {
    Write-Host "[✗] 自动配置失败，请手动执行: spss-studio-mcp configure-claude" -ForegroundColor Red
