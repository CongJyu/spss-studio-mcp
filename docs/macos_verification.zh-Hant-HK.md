...
解析為絕對路徑（以伺服器 cwd 為基準，並展開 `~`）。Windows 同樣受益——兩邊行為
都更穩健。

### 3. 自動開啟 `.spv` 檢視器
Windows 用 `os.startfile()` 開啟 SPSS Viewer。
**修正**（`spss_runner.py`）：在 macOS 以 `open` 開啟，並預設關閉，除非設定
`SPSS_OPEN_VIEWER=1`，以免 headless／批次驗證時每次成功分析都彈出 GUI。

### 4. macOS 不產出 EMF（Windows 中繼檔）
真機檢查：SPSS 32 for Mac 的 `OMS FORMAT=DOC` 匯出只在 DOCX 內寫入
`word/media/imageN.eps`，其內容實際是 **PNG 點陣圖**——完全沒有 EMF 成員。
**決定**：EMF 保留為 **Windows** 功能。在 macOS 請求 EMF 時，
`oms_image.extract_docx_emf` 會於執行後檢查 DOCX 內容並回傳明確錯誤——
「SPSS did not produce vector EMF output on macOS … request PNG or TIFF
instead」——而不會靜默降級。PNG／TIFF 是 macOS 上受支援的出版格式。

## 建議的 macOS 使用方式

- 分析工具（t 檢定／ANOVA／迴歸／中介／調節／存活分析……）：與 Windows 相同，
  全部可用。
- 圖表：使用 `PNG` 或 `TIFF`（300 dpi，可直接投稿）；**請勿使用 `EMF`**。
- 資料路徑：相對或絕對皆可（相對路徑會自動解析為絕對路徑）。
- 安裝與設定：`scripts/install_macos.sh` 及 README／QUICK_START 的「macOS」章節。
